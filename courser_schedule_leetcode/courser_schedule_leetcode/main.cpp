//
//  main.cpp
//  courser_schedule_leetcode
//
//  Created by Alps on 15/5/10.
...
public:
    bool canFinish(int numCourses, vector<vector<int> >& prerequisites) {
        ...
        vector<vector<int> >::iterator tempIter;
        int table[numCourses][numCourses];
        for (int i = 0; i < numCourses; i++) {
            memset(table[i], 0, sizeof(int) * numCourses);
        }
        for (iter = prerequisites.begin(); iter != prerequisites.end(); iter++) {
            table[(*iter)[0]][(*iter)[1]] = 1;
        }
